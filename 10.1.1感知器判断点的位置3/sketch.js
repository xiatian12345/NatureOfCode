class Perceptron{
  ...
  initWeights(){//可以在上次训练的基础上继续训练
    let wx = parseFloat(localStorage.getItem('wx')) || random(-1,1);
    let wy = parseFloat(localStorage.getItem('wy')) || random(-1,1);
    let wb = parseFloat(localStorage.getItem('wb')) || random(-1,1);
...
let targetLine;
let points = [];
let pointsLen = 1000000;//训练总数
let perceptron;
let mousePoint = null;
let trainIndex = 0;
let trainRate = 1000;//每帧训练的个数

let saveToLocalStorage = false;

let predictPoints = [];
let predictPointsLen = 500;//预测的点的个数
function setup() {
  createCanvas(800, 600);
  coordinate = new Coordinate();
  targetLine = new TargetLine(1,300);
  ...
  for(let i = 0;i < predictPointsLen;i ++){
    let x = random(-width/2,width/2);
    let y = random(-height/2,height/2);
    let p = new Point(x,y);
    p.isMouse = true;
    predictPoints.push(p);
  }
}
...
function draw() {
  ...
  if(trainIndex < pointsLen){
    for(let i = 0;i < trainRate;i ++){
      let index = trainIndex + i;
      // console.log(index);
      let trainer = new Trainer(points[index],targetLine);
      perceptron.train(trainer.inputs,trainer.answer);
    }
  }else if(!saveToLocalStorage){
    saveToLocalStorage = true;
    localStorage.setItem('wx',perceptron.weights[0]);
    localStorage.setItem('wy',perceptron.weights[1]);
    localStorage.setItem('wb',perceptron.weights[2]);
  }

  trainIndex = trainIndex+trainRate;
  ...
  for(let i = 0;i < predictPointsLen;i ++){
    let p = predictPoints[i];
    let trainer = new Trainer(p,targetLine);
    p.setCIndex(perceptron.output(trainer.inputs));
    p.display();
  }
